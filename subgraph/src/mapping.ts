...
export function handleMarketTokenMinted(event: MarketTokenMintedEvent): void {
  ...
  if (contract != null) {
    ...
    const tokenUriProp = token.uri;
    const tokenUri: string = tokenUriProp ? tokenUriProp : "";

    // Get only the IPFS CID hash from URL string.
    const tokenIPFSHash = tokenUri.substring(tokenUri.lastIndexOf("/") + 1);

    if (tokenIPFSHash) {
      const metadata = ipfs.cat(tokenIPFSHash);
      if (metadata) {
